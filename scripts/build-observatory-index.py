    ...
    try:
        ...
        for path, line_no, snap in _iter_snapshots(repo_root):
            ...
            for item in snap.get("sources", []):
                ...
                base = (ts, sid, item.get("label", sid), collector_ok, status, item.get("http_status"), item.get("latency_ms"), rel, line_no)
                obs_rows.append(base)
                if sid in PROVIDERS:
                    provider_rows.append((ts, sid, collector_ok, status, item.get("http_status"), item.get("latency_ms"), rel, line_no))
                if sid in SPACE_WEATHER:
                    summary = item.get("summary") or {}
                    weather_rows.append((
                        ts, sid, collector_ok,
                        _scale(summary, "R"), _scale(summary, "S"), _scale(summary, "G"),
                        summary.get("rows") if sid == "noaa_planetary_k_index" else None,
                        rel, line_no,
                    ))
        if obs_rows:
            con.executemany("insert into observations values (?,?,?,?,?,?,?,?,?)", obs_rows)
        if provider_rows:
            con.executemany("insert into provider_status values (?,?,?,?,?,?,?,?)", provider_rows)
        if weather_rows:
            con.executemany("insert into space_weather values (?,?,?,?,?,?,?,?,?)", weather_rows)

        con.execute("create view v_source_health as select collected_at, source_id, collector_ok, source_status, latency_ms, provenance_path from observations")
        con.execute("create view v_provider_events as select * from provider_status where collector_ok = false or source_status not in ('none','OBSERVED','All Systems Operational')")
        con.execute("create view v_space_weather as select * from space_weather where collector_ok = true")
        con.execute("""
