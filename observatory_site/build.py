...
def build_site(repo_root: Path, output: Path, base_path: str, now: datetime) -> None:
    ...
    snapshot = load_latest_snapshot(
        repo_root, now=now, freshness_budget_seconds=FRESHNESS_BUDGET_SECONDS
    )
    reports = load_reports(repo_root)
    experiments = load_experiments(repo_root)
    _write(
        output / "index.html",
        render_home(snapshot, reports, experiments, base_path=base_path),
    )
    _write(
        output / "observations/index.html",
        render_observations(snapshot, base_path=base_path),
    )
    _write(
        output / "experiments/index.html",
        render_experiments(experiments, base_path=base_path),
    )
    for exp in experiments:
        _write(
            output / f"experiments/{exp.slug}/index.html",
            render_experiment(exp, base_path=base_path),
        )
    method_path = repo_root / "docs/methodology.md"
    method_html = markdown.markdown(
        method_path.read_text(encoding="utf-8"), extensions=["fenced_code"]
    )
    _write(
        output / "method/index.html",
        render_markdown_page("Method", method_html, base_path=base_path),
    )
    ...
    _write(
        ...
        render_markdown_page("About / collaboration", about, base_path=base_path),
    )
