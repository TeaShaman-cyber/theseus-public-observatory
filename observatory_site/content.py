...
from .model import ExperimentSummary, ObservationSnapshot, ReportSummary, SourceObservation
...
def load_latest_snapshot(repo_root: Path, *, now: datetime, freshness_budget_seconds: int) -> ObservationSnapshot:
    ...
    for item in data.get("sources", []):
        ...
        if collector_ok:
            source_status = str(summary.get("indicator") or summary.get("description") or "OBSERVED")
        else:
    ...
    return ObservationSnapshot(collected_at=collected_at, freshness=freshness, age_seconds=age, sources=tuple(sources))
...
def load_reports(repo_root: Path) -> list[ReportSummary]:
    ...
    for path in sorted((repo_root / "reports").glob("*.md"), reverse=True):
        ...
        title = next((line[2:].strip() for line in first if line.startswith("# ")), path.stem)
        reports.append(ReportSummary(slug=path.stem, title=title, path=str(path.relative_to(repo_root))))
    return reports
